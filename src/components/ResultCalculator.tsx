...
import { Calculator, TrendingUp, Award, BarChart3, Plus, Trash2 } from "lucide-react";
...
export function ResultCalculator() {
  const [currentSemesterGrades, setCurrentSemesterGrades] = useState<CourseGrade[]>([
    { id: '1', courseCode: 'CSC 301', courseTitle: 'Database Systems', units: 3, grade: 'A', points: 5 },
    { id: '2', courseCode: 'CSC 305', courseTitle: 'Software Engineering', units: 3, grade: 'B', points: 4 },
    { id: '3', courseCode: 'STA 301', courseTitle: 'Applied Statistics', units: 3, grade: 'A', points: 5 },
  ]);
  ...
  const calculateSemesterGPA = (grades: CourseGrade[]) => {
    const totalPoints = grades.reduce((sum, course) => sum + (course.points * course.units), 0);
    const totalUnits = grades.reduce((sum, course) => sum + course.units, 0);
    return totalUnits > 0 ? (totalPoints / totalUnits).toFixed(2) : '0.00';
  };

  const calculateCumulativeGPA = () => {
    const allSemesterPoints = semesterHistory.reduce((total, sem) => total + (sem.gpa * sem.totalUnits), 0);
    const currentSemesterPoints = currentSemesterGrades.reduce((sum, course) => sum + (course.points * course.units), 0);
    
    const allSemesterUnits = semesterHistory.reduce((total, sem) => total + sem.totalUnits, 0);
    const currentSemesterUnits = currentSemesterGrades.reduce((sum, course) => sum + course.units, 0);
  ...
  const addNewCourse = () => {
    if (newCourse.courseCode && newCourse.courseTitle && newCourse.units && newCourse.grade) {
  ...
  const removeCourse = (id: string) => {
    setCurrentSemesterGrades(currentSemesterGrades.filter(course => course.id !== id));
  };

  const updateCourseGrade = (id: string, grade: string) => {
    const gradeInfo = gradeScale.find(g => g.grade === grade);
    setCurrentSemesterGrades(currentSemesterGrades.map(course => 
      course.id === id 
  ...
  const totalUnitsEarned = semesterHistory.reduce((total, sem) => total + sem.totalUnits, 0) + 
  ...
  return (
    <div className="flex-1 overflow-auto">
      <div className="p-6">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-semibold mb-2">Results & GPA Calculator</h1>
          <p className="text-muted-foreground">Calculate your semester and cumulative GPA</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-blue-600">{currentSemesterGPA}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-600">{cumulativeGPA}</div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Total Units Earned</CardTitle>
              <Award className="h-4 w-4 text-purple-600" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-purple-600">{totalUnitsEarned}</div>
              <p className="text-xs text-muted-foreground">Credit units completed</p>
            </CardContent>
          </Card>
        </div>
          ...
          <TabsContent value="calculator" className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Enter course details and grade to calculate GPA</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Semester GPA: <span className="font-bold text-primary">{currentSemesterGPA}</span>
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  {currentSemesterGrades.map((course) => (
                    <div key={course.id} className="flex items-center justify-between p-4 rounded-lg border">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <Select value={course.grade} onValueChange={(value) => updateCourseGrade(course.id, value)}>
                        ...
                        <Badge variant={course.points >= 4 ? "default" : course.points >= 3 ? "secondary" : "destructive"}>
                          {course.points} pts
                        </Badge>
                        <Button
                          variant="ghost"
                  ...
                  <div className="mt-6 p-4 bg-primary/5 rounded-lg border border-primary/20">
                    <div className="grid grid-cols-3 gap-4 text-center">
                      ...
                      <div>
                        ...
                        <p className="text-lg font-bold">
                          {currentSemesterGrades.reduce((sum, course) => sum + (course.points * course.units), 0)}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Semester GPA</p>
                        <p className="text-lg font-bold text-primary">{currentSemesterGPA}</p>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <CardContent>
                  ...
                  {semesterHistory.map((semester, index) => (
                    <div key={index} className="flex items-center justify-between p-4 rounded-lg border">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{semester.totalUnits} units completed</p>
                      </div>
                      <div className="flex items-center gap-3">
                        <Badge variant="secondary">{semester.totalUnits} Units</Badge>
                        <Badge 
                          variant={semester.gpa >= 4.0 ? "default" : semester.gpa >= 3.0 ? "secondary" : "destructive"}
                ...
                <div className="mt-6 p-4 bg-green-50 rounded-lg border border-green-200">
                  <div className="flex justify-between items-center">
                    <span className="font-medium text-green-800">Cumulative GPA</span>
                    <span className="text-xl font-bold text-green-800">{cumulativeGPA}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
                  ...
                  {gradeScale.map((grade) => (
                    <div key={grade.grade} className="p-4 rounded-lg border">
                      <div className="flex items-center justify-between mb-2">
                        <Badge 
                          variant={grade.points >= 4 ? "default" : grade.points >= 3 ? "secondary" : "destructive"}
                      ...
                      <p className="text-sm text-muted-foreground">Score: {grade.range}%</p>
                    </div>
                  ))}
                </div>
                
                <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                  <h4 className="font-medium text-blue-800 mb-2">GPA Calculation Formula</h4>
                  <p className="text-sm text-blue-700">
                    GPA = (Sum of Grade Points × Units) ÷ Total Units
                  </p>
                  <p className="text-xs text-blue-600 mt-1">
                    Example: If you get A(5pts) in 3-unit course and B(4pts) in 2-unit course: 
                    GPA = (5×3 + 4×2) ÷ (3+2) = 23 ÷ 5 = 4.6
                  </p>
                </div>
              </CardContent>
            </Card>
